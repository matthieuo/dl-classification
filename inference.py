...
class tomato_inf:
    def __init__(self,check_):
        ...
        self.output_logits = tf.nn.softmax(models.foodv_test(self.img_feed,reg_val=0.0,is_train=False,dropout_p = 1.0))
    ...
    def has_tomatoes(self,im_path):
        ...
        pred = self.sess.run(self.output_logits,
                             feed_dict={self.img_feed:im.eval(session=self.sess)})
...
if __name__ == '__main__':
    ...
    group_genera.add_argument("-img", "--img_path", help="Path to the image",required=True)
    group_genera.add_argument("-cp", "--checkpoint", help="Path to the checkpoint",required=True)
